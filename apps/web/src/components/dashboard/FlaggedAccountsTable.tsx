...
import { CURRENCY_SYMBOL, getCurrencySymbol } from "@/utils/currency";
...
import AccountProfileSidePanel from "./AccountProfileSidePanel";
            ...
            <tbody>
              {filteredAccounts.map((acct) => (
                <tr
                  ...
                  className="border-b border-outline-variant/10 hover:bg-surface-container-high/30 transition-colors text-xs text-on-surface align-middle cursor-pointer"
                >
                  {/* Account Number */}
                  <td className="p-4 font-label-mono font-bold text-primary">
                    {acct.account_number}
                  </td>
                  ...
                  <td className="p-4 text-right font-bold font-label-mono text-primary">
                    {getCurrencySymbol(acct.currency)}{acct.balance.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
