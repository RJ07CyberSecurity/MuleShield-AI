  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        ...
        if (caseRes?.success && caseRes.data?.customer_id) {
          custId = caseRes.data.customer_id;
        }
        ...
        const acctsRes = await apiClient.get<any>("/api/v1/accounts");
        if (acctsRes?.success && Array.isArray(acctsRes.data) && acctsRes.data.length > 0) {
          const validAccounts = acctsRes.data.filter((a: any) =>
            a.account_number &&
            /\d/.test(a.account_number) &&
            !["salary", "deposit", "generated", "account", "not found", "unknown"].some(w => a.account_number.toLowerCase().includes(w))
          );
          const acct = validAccounts.length > 0 ? validAccounts[0] : acctsRes.data[0];
          setAccount(acct);

          if (acct.id) {
            const profRes = await apiClient.get<any>(`/api/v1/accounts/${acct.id}/profile`);
            if (profRes?.success && profRes.data) {
              setAccount(profRes.data);
              if (profRes.data.customer) setCustomer(profRes.data.customer);
            }
