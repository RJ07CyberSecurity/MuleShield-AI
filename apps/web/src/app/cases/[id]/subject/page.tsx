...
import { UserCheck, ShieldCheck, FileSpreadsheet, ArrowUpRight, ArrowDownLeft, CreditCard, Phone, Mail, Calendar, RefreshCw } from "lucide-react";
...
export default function SubjectProfilePage({ params }: PageProps) {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        ...
        if (selectedStmtId) {
          ...
          try {
            const txRes = await apiClient.get<any>(`/api/v1/dashboard/critical-alerts?ingestion_id=${encodeURIComponent(selectedStmtId)}`);
    ...
    fetchProfile();
  }, [caseId, selectedStmtId, statements]);
  ...
  const totalVolume = summaryData?.total_volume || 4250000;
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-xs font-label-mono text-on-surface-variant uppercase tracking-wider">Loading Subject Financial Profile...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-3 space-y-6">
          <div className="p-6 rounded-2xl border border-outline-variant/20 bg-surface-container-low/90 space-y-5 text-center relative overflow-hidden shadow-lg">
            ...
            <button
              onClick={() => addToast(`Generated compliance dossier for ${activeProfile.name}`, "success")}
